...
class Crypt {
  ...
  */
  ...
  Crypt.md5(String key, {int? rounds, String? salt}) {
    final c = _md5Algorithm(crypto.md5, 16, key, rounds: rounds, salt: salt);

    final result = StringBuffer();
    _encode_3bytes(result, c[0], c[6], c[12]);
    _encode_3bytes(result, c[1], c[7], c[13]);
    _encode_3bytes(result, c[2], c[8], c[14]);
    _encode_3bytes(result, c[3], c[9], c[15]);
    _encode_3bytes(result, c[4], c[10], c[5]);
    _encode_3bytes(result, c[11]);
  ...
  //----------------------------------------------------------------
  // Implementation of the algorithm used for md5 crypt.

  List<int> _md5Algorithm(crypto.Hash md5, int hl, String key,
      {int? rounds, String? salt}) {
    final pwdBytes = List<int>.from(key.codeUnits);
    ...
    if (salt == null) {
      ...
      if (maxShaLength < salt.length) {
        salt = salt.substring(0, maxShaLength);
      }
    ...
    final br = md5.convert(dataB).bytes; // Step 8
    ...
    if (0 < count) {
      dataA.addAll(br.sublist(0, count)); // Step 10
    }
    ...
    for (var bits = key.length; bits != 0; bits >>= 1) {
      if (bits & 0x01 != 0) {
        ...
        dataA.addAll(List<int>.from(key[0].codeUnits));
      }
    }
    final digestA = md5.convert(dataA).bytes; // Step 12
    ...
    for (var r = 0; r < rounds; r++) {
      ...
      if ((r % 2) == 1) {
        dataC.addAll(pwdBytes);
      } else {
        dataC.addAll(running);
      }

      if ((r % 3) != 0) {
        dataC.addAll(saltBytes);
      }
      if ((r % 7) != 0) {
        dataC.addAll(pwdBytes);
      }
      ...
      running = md5.convert(dataC).bytes;
    }
  ...
  bool match(String value) {
    ...
    switch (_type) {
      ...
      case idSha512:
        ...
        break;
      case idMd5:
        that = Crypt.md5(value, rounds: _rounds, salt: _salt);
        break;
      default:
